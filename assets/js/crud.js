...
const CRUD = {
  ...
  def(moduleId){
    ...
    canWrite(moduleId) {
    const role = String((window.App && App.currentRole) || '').toLowerCase().trim();
    if (['super_admin','admin','principal','proprietor','head_teacher','bursar'].includes(role)) return true;
    if (['staff','teacher'].includes(role)) {
      const adminOnly = ['academic_setup','approvals','admin_data','payroll','hr','compliance','storage','settings','finance','inventory','promotion','alumni','departments','directory','staff'];
      return !adminOnly.includes(String(moduleId||'').replace(/-/g,'_'));
    }
